...
<div class="page-wrapper"><main class="addon-form-page">
    <header class="addon-form-head"><div><p class="addon-eyebrow">Qo‘shimcha xizmat</p><h1>Qo‘shimcha xizmat qo‘shish</h1><p>Asosiy xizmat uchun mijoz tanlashi mumkin bo‘lgan qo‘shimcha variantni yarating.</p></div><a class="addon-back" href="{{ route('superadmin.service.index') }}"><i class='bx bx-left-arrow-alt'></i> Katalogga qaytish</a></header>
    <form action="{{ route('superadmin.addon.store', $service_id) }}" method="POST" class="addon-card">@csrf
        ...
        <div class="addon-grid">
            <div class="addon-field"><label for="addon_name">Qo‘shimcha xizmat nomi</label><input id="addon_name" name="name" value="{{ old('name') }}" placeholder="Masalan: Tezkor tarjima" required></div>
            <div class="addon-field"><label for="addon_price">Qo‘shimcha narx <span>(so‘m)</span></label><input id="addon_price" type="number" min="0" step="0.01" name="price" value="{{ old('price') }}" placeholder="50000" required></div>
            <div class="addon-field"><label for="addon_deadline">Qo‘shimcha muddat <span>(kun)</span></label><input id="addon_deadline" type="number" min="0" step="1" name="deadline" value="{{ old('deadline') }}" placeholder="1" required></div>
            <div class="addon-field addon-field--wide"><label for="addon_description">Qisqacha izoh <span>(ixtiyoriy)</span></label><textarea id="addon_description" name="description" placeholder="Qo‘shimcha xizmat nimani o‘z ichiga olishini yozing.">{{ old('description') }}</textarea></div>
        </div>
        <div class="addon-actions"><a href="{{ route('superadmin.service.index') }}" class="addon-cancel">Bekor qilish</a><button type="submit" class="addon-save"><i class='bx bx-check'></i> Qo‘shimchani saqlash</button></div>
    </form>
</main></div>
@endsection
