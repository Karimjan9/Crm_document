        ...
        confirmInfo.innerHTML = `
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="mb-1"><strong>Apostil:</strong> ${escapeHtml(directionType)}</p>
                    <p class="mb-1"><strong>Konsullik:</strong> ${consulateChecked ? "Ha" : "Yo'q"}</p>
                    ${consulateChecked && legalization !== 'Tanlanmagan' ? `<p class="mb-1"><strong>Legallashtirish:</strong> ${escapeHtml(legalization)}</p>` : ''}
    ...
    updateConfirm() {
        ...
        if (processPayload.processMode === 'consul') {
            ...
            if (processPayload.consul.consul_id) {
                processHtml += `<p class="mb-1"><strong>Konsullik:</strong> ${escapeHtml(consulName)}</p>`;
            }
            if (processPayload.legalization.id) {
                processHtml += `<p class="mb-1"><strong>Legalizatsiya:</strong> ${escapeHtml(legalizationName)}</p>`;
            }
        }
